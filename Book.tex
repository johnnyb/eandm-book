\documentclass{book}

\usepackage[margin=1in, paperwidth=7.5in, paperheight=9.25in]{geometry}

%% bibliographies for each chapter, as a section in the chapter

\usepackage[sectionbib]{chapterbib} 

%% For Bio photos
\usepackage{floatflt}

%% From Ewert
\usepackage{fullpage} 
\usepackage{graphicx} 
\usepackage[numbers]{natbib} 
\usepackage{todonotes} 
\usepackage[stable]{footmisc}

%% From Holloway
\usepackage{algorithmic}
\usepackage{algorithmextra}
\usepackage{lscape}
%% \usepackage{graphicx}
%% \usepackage{psfrag}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amsthm}
% \usepackage{cite}
\usepackage{makeidx}
\usepackage{longtable}
\usepackage{amssymb}
\usepackage{balance}
%% \usepackage[lastname]{authorindex}
%% \usepackage{subfigure}
\usepackage{listings}
%% \usepackage{color}
%% \usepackage{rotating}
% declare the path(s) where your graphic files are
%% \graphicspath{{./}}

%% From Bartlett2
\usepackage{float}

%% For Sich
\usepackage[T2A,T1,OT1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage[russian,english]{babel}

\DeclareGraphicsExtensions{.eps,.png,.pdf}
\newenvironment{abstract}{\section*{Abstract}}{}
\newenvironment{introquote}{\hfill}{\vspace{2pc}}
\newenvironment{introquoteref}{\hfill}{\vspace{2pc}}

\renewcommand*\thesection{\arabic{section}}

%% These are for Sich's article, in case we want to standardize these
\newcommand\sbf[1]{\textbf{#1}}
\newcommand\semph[1]{\textit{#1}}
\newcommand\sunder[1]{\underline{#1}}
\newcommand\btitle[1]{\emph{#1}}
\newcommand\fword[1]{\emph{#1}}
\newcommand\sundertitle[1]{\underline{#1}}
\newcommand\foreign[1]{\emph{#1}}

\newcommand\biophoto[1]{\begin{floatingfigure}[p]{2.25in}\includegraphics[width=2in]{#1}\end{floatingfigure}}
\newcommand\biosection[2]{\newpage \section*{#1} \biophoto{#2}}

%% \newcommand{\myvec}[1]{\vec{#1}} %% how to define a new command

\setcounter{tocdepth}{1}

%% This keeps us from having overlong lines
\sloppy

\makeindex

\begin{document}
\title{Engineering and the Ultimate}
% \subtitle{An Interdisciplinary Investigation of Order and Design in Nature and Craft}
\author{Jonathan Bartlett and Dominic Halsmer}
\date{April 2013}
\maketitle

\tableofcontents

\include{Introduction}

\include{Mignea1}
\include{Mignea2}
\include{Mignea3}

\include{Hall}

\include{Ewert}

\include{Bartlett1}
\include{Bartlett2}

\include{Holloway}

\include{Sich}

\include{AboutAuthors}

\cleardoublepage
\addcontentsline{toc}{chapter}{Index}
\printindex


\end{document}
